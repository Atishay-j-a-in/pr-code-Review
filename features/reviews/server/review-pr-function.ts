...
export const reviewPullRequest = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        const chunks = await step.run("break-down code", async () => {
            const files = await getPullRequestFiles(
                pullRequest.installationId, pullRequest.repoFullName, pullRequest.prNumber
            );
        ...
        if (chunks.length === 0) {
            ...
            return { pullRequestId, status: "reviewed", message: "No code changes to review." }
        }

        //  PR namespace isolates this diff from other PRs and from repo-wide sync data(add for pinecone db)
